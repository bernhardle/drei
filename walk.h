...
// $Source: /home/dfs/yc20/yc23/3/hier/developers/RCS/walk.h,v $
// $Revision: 2.2 $
// $Name:  $
// $State: Exp $
...
static volatile char RCSId_walk_h [] = "$Id: walk.h,v 2.2 1998/10/29 11:39:18 yc23 Exp yc23 $" ;
...
template < class A > class VectorAlign : public IteratorSTI < A >, public MyAlloc {
  ...
  typename vector < pointer_t > :: iterator _curr, _ahead ;
  int _cnt ;
...
template < class A > void VectorAlign < A > :: first () {
  ...
  for ((_ahead = _curr) == _it.end () ? 0 : _ahead ++ ; (_ahead == _it.end () ? 0 : ((*_ahead)->first (), (*_ahead)->done ())) ; _ahead ++) ;
  return ;
}
...
template < class A > void VectorAlign < A > :: next () {
  (*_curr)->next () ;
  if ((*_curr)->done ())
    for (_ahead == _it.end () ? 0 : (_curr = _ahead, _ahead ++) ; (_ahead == _it.end () ? 0 : ((*_ahead)->first (), (*_ahead)->done ())) ; _ahead ++) ;
  return ;
}
