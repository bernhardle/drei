	...
	// $Source: /home/dfs/yc20/yc23/3/hier/optimized/RCS/gitter_sti.cc,v $
	// $Revision: 2.2 $
	// $Name:  $
	// $State: Exp $
...
static volatile char RCSId_gitter_sti_cc [] = "$Id: gitter_sti.cc,v 2.2 1998/10/29 11:39:18 yc23 Exp yc23 $" ;
...
void Gitter :: backup (const char * fileName) {

	// Die Konstruktion des Backuo und Restore ist folgendermassen gedacht:
  ...
  assert (debugOption (20) ? (cout << "**INFO Gitter :: backup (const char * = \"" << fileName << "\") " << endl, 1) : 1) ;
  ofstream out (fileName) ;
  if (!out) {
    cerr << "**WARNUNG (IGNORIERT) Gitter :: backup (const char *) Fehler beim Anlegen von < " 
         << (fileName ? fileName : "null") << " >" << endl ;
  } else {
    FSLock lock (fileName) ;
    backup (out) ;
    container ().backup (fileName) ;
  }
  return ;
}

void Gitter :: backupCMode (const char * fileName) {
  assert (debugOption (20) ? (cout << "**INFO Gitter :: backupCMode (const char * = \"" << fileName << "\") " << endl, 1) : 1) ;  
  ofstream out (fileName) ;
  if (!out) {
    ...
    backupCMode (out) ;
    container ().backupCMode (fileName) ;
  }
  return ;
}

void Gitter :: restore (const char * fileName) {

	// Erkl"arung siehe Gitter :: backup (const char *) in gitter_sti.cc ;

  assert (debugOption (20) ? (cout << "**INFO Gitter :: restore (const char * = \"" << fileName << "\") " << endl, 1) : 1) ;
  ifstream in (fileName) ;
  if (!in) {
    cerr << "**WARNUNG (IGNORIERT) Gitter :: restore (const char *) Fehler beim \"Offnen von < " 
         << (fileName ? fileName : "null") << " > " << endl ;
  } else {
    restore (in) ;
  }
